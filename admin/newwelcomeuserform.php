...
if (isset($_POST['submit'])) {
    ...
    $select_chech = mysqli_query($con, "SELECT * FROM welcomeuser_tbl WHERE email='".trim($_POST['email'])."' OR phonenumber='".trim($_POST['phonenumber'])."'");

    if (mysqli_num_rows($select_chech) > 0) {
        echo "<script>
        alert('Your phonenumber and email is already exists check');document.location ='add_new_admin_user.php';
        </script>";
    }
elseif ($password != $cpass) {

    echo "<script>
    alert('Password are not Match');document.location ='add_new_welcome_user.php';
    </script>";
}
...
elseif (mysqli_num_rows($select_chech)==0) { 
    ...
    $status=0;
    $insert=mysqli_query($con,"INSERT INTO `welcomeuser_tbl`(`firstname`, `lastname`, `phonenumber`, `email`, `password`, `status`) 
    VALUES ('$firstname','$lastname','$phonenumber','$email','$password','$status')");
    if ($insert) {
        
        echo "<script>
        alert('New Account  is  created Successfully');document.location ='add_new_welcome_user.php';
        </script>";
    }else {
        echo "<script>
        alert('New Account  is  created Successfully');document.location ='add_new_welcome_user.php';
    ...
    echo "<script>
    alert('Your phonenumber or email is already exists please check');document.location ='add_new_welcome_user.php.php';
    </script>";
}
}
?>
