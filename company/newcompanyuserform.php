...
if (isset($_POST['submit'])) {
    ...
    $select_chech = mysqli_query($con, "SELECT * FROM companyuser_tbl WHERE email='".trim($_POST['email'])."' OR phonenumber='".trim($_POST['phonenumber'])."'");
...
elseif (mysqli_num_rows($select_chech)==0) { 
    ...
    $status=0;
    $insert=mysqli_query($con,"INSERT INTO `companyuser_tbl`(`firstname`, `lastname`, `phonenumber`, `email`, `password`, `status`) 
    VALUES ('$firstname','$lastname','$phonenumber','$email','$password','$status')");
    if ($insert) {
        
        echo "<script>
        alert('New Account  is  created Successfully');document.location ='add_new_company_user.php';
        </script>";
    }else {
        echo "<script>
        alert('New Account  is  created Successfully');document.location ='add_new_company_user.php';
    ...
    echo "<script>
    alert('Your phonenumber or email is already exists please check');document.location ='add_new_company_user.php';
    </script>";
}
}
?>
