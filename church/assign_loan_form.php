...
<div class="card-body">
  ...
  <form class="forms-sample" method="post" enctype="multipart/form-data" class="form-horizontal">
    ...
    $eid=$_POST['edit_id'];
     ...
     $sql="SELECT * FROM tbl_borrow LEFT JOIN member_tbl ON member_tbl.member_id= tbl_borrow.userID  WHERE tbl_borrow.status='YES' AND tbl_borrow.id=:eid";          
    $query = $dbh -> prepare($sql);
    $query-> bindParam(':eid', $eid, PDO::PARAM_STR);
    $query->execute();
    ...
    if($query->rowCount() > 0)
    {
      ...
      { 
        ...
        <div class="form-group">
          ...
          <input type="hidden" name="servicename" class="form-control" id="servicename" value="<?php  echo $row->id;?>"required>
        </div>
        <div class="form-group">
